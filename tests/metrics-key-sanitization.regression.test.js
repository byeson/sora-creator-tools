...
test('sanitizers preserve deduped remix post ids when a metrics signal is present', () => {
  const sanitizeMetricsItem = buildContentSanitizerHarness();
  const sanitizeMetricsSnapshot = buildBackgroundSanitizerHarness();
  const payload = {
    userKey: 'h:alice.sora',
    postId: 's_parent',
    remix_post_ids: ['s_child_a', 's_child_b', 's_child_a'],
  };
...
test('sanitizers drop malformed remix post ids but keep the post payload', () => {
  const sanitizeMetricsItem = buildContentSanitizerHarness();
  const sanitizeMetricsSnapshot = buildBackgroundSanitizerHarness();
  const payload = {
    userKey: 'h:alice.sora',
    postId: 's_parent',
    remix_post_ids: ['s_child_a', '', 'bad id!', null, 's_child_b'],
  };
